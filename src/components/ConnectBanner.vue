...
<v-banner single-line outlined rounded class="mt-8" v-else-if="this.device.isConnected && $root.$data.product !== null">
    <v-icon slot="icon" color="green darken-3">mdi-check</v-icon>
    <span class="text-body-1 green--text text--darken-3">Connected to
        {{ getDeviceName($root.$data.product) }}</span >
</v-banner>
